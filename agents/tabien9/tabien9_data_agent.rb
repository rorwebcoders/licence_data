...
class Tabien9Mailer < ActionMailer::Base
...
class Tabien9DatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/tabien9_data_builder_agent.log", 'weekly')
          ...
          listings.each do |each_list|
            ...
            listings_1.each_with_index do |each_data, ind|
              begin
                ...
                if each_data.to_s.include?"https://www.9tabien.com/img/purple.png"
                  color = 'white special' 
                elsif each_data.to_s.include?"https://www.9tabien.com/img/white.png"
                  color = 'white'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/phuket.png"
                  color = 'phuket'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/choburi.jpg"
                  color = 'chonburi'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/gold.png"
                  color = 'gold'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/ggreen.png"
                  color = 'green special'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/gblue.png"
                  color = 'blue special'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/blue.png"
                  color = 'blue basic'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/rayong.jpg"
                  color = 'rayong'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/songkra.jpg"
                  color = 'songkla'
                elsif each_data.to_s.include?"https://www.9tabien.com/img/nara.png"
                  color = 'nara'
                else
                  color = ''
                end
                ...
                if exist_data.count == 0
                  ...
                  results = Tabien9Detail.create(:date_created => date_created, :url => each_url, :license_group => license_group, :license_number => license_number, :price => price, :location => location, :license_status => status, :color => color, :processing_status => '')
                end
              rescue Exception => e
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabien9_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = Tabien9Detail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = Tabien9Detail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = Tabien9Detail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            Tabien9Detail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => status, :location => location, :date_created => s_current_date, :processing_status => processing_status, :price_status => '0')
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabien9_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = Tabien9Detail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = Tabien9Detail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = Tabien9Detail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          Tabien9Detail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby tabien9_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new tabien9 agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = Tabien9DatatBuilderAgent.new(options)
