...
class AttabienMailer < ActionMailer::Base
...
class AttabienDatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/attabien_data_builder_agent.log", 'weekly')
  ...
  def start_processing
    begin
      if $db_connection_established
        urls = ["https://attabien.com"]
        urls.each do |each_url|
          doc = Nokogiri::HTML(RestClient.get(each_url).body)
          date_created = ((Date.today)).strftime('%Y-%m-%d')
          
          listings = doc.css('div.tabien')
          listings.each_with_index do |each_list, ind1|
            license_group = ""
            if each_list.parent.xpath("preceding-sibling::div").css('h2.fh5co-section-lead11').count == 1
              license_group = each_list.parent.xpath("preceding-sibling::div").css('h2.fh5co-section-lead11').text.strip() rescue ""
            else
              license_group = each_list.parent.xpath("preceding-sibling::div").css('h2.fh5co-section-lead11').last.text.strip() rescue ""
            end

            listings_1 = each_list.css('div.product_box')
            ...
            listings_1.each_with_index do |each_data, ind|
              begin
                ...
                exist_data = AttabienDetail.where("date_created = '#{date_created}' and license_number = '#{license_number}' and url = '#{each_url}'")
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from attabien_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = AttabienDetail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = AttabienDetail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = AttabienDetail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            AttabienDetail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => status, :location => location, :date_created => s_current_date, :processing_status => processing_status, :price_status => '0')
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from attabien_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = AttabienDetail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = AttabienDetail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = AttabienDetail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          AttabienDetail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby attabien_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new attabien agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = AttabienDatatBuilderAgent.new(options)
