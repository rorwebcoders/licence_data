...
class TabienhengMailer < ActionMailer::Base
...
class TabienhengDatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/tabienheng_data_builder_agent.log", 'weekly')
  ...
  def start_processing
    begin
      if $db_connection_established
        urls = ["http://www.tabienheng.com/"]
        urls.each do |each_url|
          doc = Nokogiri::HTML(RestClient.get(each_url).body)
          date_created = ((Date.today)).strftime('%Y-%m-%d')
          
          listings = doc.css('div.post')[1..-1]
          
          listings.each_with_index do |each_list, ind1|
              license_group = each_list.css('div.post-title').css('h2').text.split(":").last.strip rescue ""
              

            listings_1 = each_list.css('div.divFrameTop').css("div[style]")
            
            listings_1.each_with_index do |each_data, ind|
              begin
                
                if each_data.to_s.include?'tabien2'
                  license_number = each_data.css('.tabien').css('a').text.strip() + each_data.css('.tabien2').text.strip() rescue ""
                else
                  license_number = each_data.css('.tabien').css('a').text.strip() rescue ""
                end
                price = each_data.css('.price').text.split("฿").first.strip() rescue ""
                link = "http://www.tabienheng.com/" + each_data.css(".tabien").css('a').attr('href').value rescue ""
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabienheng_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TabienhengDetail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = TabienhengDetail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = TabienhengDetail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            TabienhengDetail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => status, :location => location, :date_created => s_current_date, :processing_status => processing_status, :price_status => '0')
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabienheng_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TabienhengDetail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = TabienhengDetail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = TabienhengDetail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          TabienhengDetail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby tabienheng_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new tabienheng agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = TabienhengDatatBuilderAgent.new(options)
