...
class Tabien999Mailer < ActionMailer::Base
...
class Tabien999DatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/tabien999_data_builder_agent.log", 'weekly')
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabien999_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = Tabien999Detail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = Tabien999Detail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = Tabien999Detail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            Tabien999Detail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => status, :location => location, :date_created => s_current_date, :processing_status => processing_status, :price_status => '0')
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tabien999_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = Tabien999Detail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = Tabien999Detail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = Tabien999Detail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          Tabien999Detail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby tabien999_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new tabien999 agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = Tabien999DatatBuilderAgent.new(options)
