...
class TeeneetabienMailer < ActionMailer::Base
...
class TeeneetabienDatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/teeneetabien_data_builder_agent.log", 'weekly')
  ...
  def start_processing
    begin
      if $db_connection_established
        urls = ["https://www.teeneetabien.com/"]
        urls.each do |each_url|
          doc = Nokogiri::HTML(RestClient.get(each_url).body)
          # byebug
          listings = doc.css('div.content-top')[1..-1]
          date_created = ((Date.today)).strftime('%Y-%m-%d')

          listings.each do |each_list|
            license_group = ""
            license_group = each_list.css('h4').text.strip rescue ""

            listings_1 = each_list.css('div.col-md-2')
            ...
            listings_1.each_with_index do |each_data, ind|
              ...
              details_url = "https://www.teeneetabien.com#{each_data.css('a').attr('href').value}"
              uri = URI.escape(details_url)
              ...
              if exist_data.count == 0
                ...
                results = TeeneetabienDetail.create(:date_created => date_created, :url => each_url, :license_group => license_group, :license_number => license_number, :price => price, :location => location, :license_status => status, :color => color, :processing_status => '')
              end
              # break if ind >= 10
            end
            # break
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from teeneetabien_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TeeneetabienDetail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = TeeneetabienDetail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = TeeneetabienDetail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            TeeneetabienDetail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => license_status, :location => location, :date_created => date_created, :processing_status => processing_status)
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from teeneetabien_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TeeneetabienDetail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = TeeneetabienDetail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = TeeneetabienDetail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          TeeneetabienDetail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby teeneetabien_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new teeneetabien agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = TeeneetabienDatatBuilderAgent.new(options)
