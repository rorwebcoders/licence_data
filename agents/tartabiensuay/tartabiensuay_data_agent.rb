...
class TartabiensuayMailer < ActionMailer::Base
...
class TartabiensuayDatatBuilderAgent
  ...
  def create_log_file
    ...
    $logger = Logger.new("#{File.dirname(__FILE__)}/logs/tartabiensuay_data_builder_agent.log", 'weekly')
  ...
  def start_processing
    begin
      if $db_connection_established
        urls = ["https://tartabiensuay.com"]
        urls.each do |each_url|
          doc = Nokogiri::HTML(RestClient.get(each_url).body)
          date_created = ((Date.today)).strftime('%Y-%m-%d')
          
          listings = doc.css('div.row')[4..-1]
          
          listings.each_with_index do |each_data,ind|
            license_group = each_data.css("div[class*=col-md-12]").text.strip rescue ""
            
          listings_1 = each_data.next_element.css("div[class*=col-md-3]")
          listings_1.each_with_index do |each_list,ind|
              ...
              begin
                ...
                license_number = each_list.css('textnum').text.strip() rescue ""
                ...
                exist_data = TartabiensuayDetail.where("created_at = '#{date_created}' and license_number = '#{license_number}' and url = '#{each_url}'")
                  ...
                  if exist_data.count == 0
                    ...
                    results = TartabiensuayDetail.create(:date_created => date_created, :url => each_url, :license_group => license_group, :license_number => license_number, :price => price, :location => location, :license_status => status, :color => '', :processing_status => '')
                  end
  ...
  def update_status
    ...
    results5 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tartabiensuay_details where processing_status = '' order by date_created desc ")
    ...
    results5.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TartabiensuayDetail.where("url = '#{res["url"].to_s}' and date_created = '#{old_data}'")
        if a.count > 0
    ...
    previous_availability_check.each do |k,v|
      v.each do |s|
        ...
        results = TartabiensuayDetail.where("url = '#{k}' and date_created = '#{s_previous_available_date}'")
        results.each do |res|
          ...
          results_current = TartabiensuayDetail.where("url = '#{k}' and license_number = '#{license_number}' and date_created = '#{s_current_date}'")
          if results_current.count == 0
            processing_status = "Removed"
            # byebug
            TartabiensuayDetail.create(:url => k, :license_group => license_group, :license_number => license_number, :price => price, :license_status => status, :location => location, :date_created => s_current_date, :processing_status => processing_status, :price_status => '0')
          end
        end
      end
    end
    ...
    results1 = ActiveRecord::Base.connection.exec_query("select distinct(Concat(date_created,url)),date_created,url from tartabiensuay_details where processing_status = '' order by date_created desc ")
    ...
    results1.collect { |res|
      ...
      while @i < @num  do
        ...
        a = TartabiensuayDetail.where("url = '#{res["url"].to_s}' and date_created = '#{Date.parse(old_data).strftime('%Y-%m-%d')}'")
        if a.count > 0
          previous_available_date = old_data
          break
        end
    ...
    previous_status_check.each do |k,v|
      v.each do |s|
        ...
        results = TartabiensuayDetail.where("processing_status = '' and date_created = '#{s_current_date}' ")
        results.each do |res|
          id = res["id"]
          license_number = res["license_number"]
          location = res["location"]
          ...
          color = res["color"]
          license_group = res["license_group"]
          results_old = TartabiensuayDetail.where("license_number = '#{license_number}' and date_created = '#{s_previous_available_date}' and processing_status != 'Removed'")
          ...
          TartabiensuayDetail.where("id = '#{id}'").update_all(:processing_status => processing_status)
        end
      end
    end
  end
...
optparse = OptionParser.new do|opts|
  ...
  # of the help screen.
  opts.banner = "Usage: ruby tartabiensuay_data_agent.rb [options]"
  ...
  opts.on( '-e', '--env ENVIRONMENT', 'Run the new tartabiensuay agent for building the projects' ) do |env|
    options[:env] = env
  end
...
newprojects_agent = TartabiensuayDatatBuilderAgent.new(options)
